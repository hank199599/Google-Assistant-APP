   ...
   conv.ask(new Suggestions('🎮 開始挑戰','👋 掰掰'));
	   ...
	   input_word=""; last_word=""; first_word="";worngcount=0;
		 ...
		 if (conv.user.last.seen) {
	  	conv.ask(new SimpleResponse({               
		speech: `<speak><p><s>歡迎回來!</s><break time="0.3s"/><s>在你開始接龍前，可以說<break time="0.2s"/>重新開始<break time="0.2s"/>讓我重新想一個詞彙作為開頭。</s><s>當你想結束挑戰時，可以說<break time="0.2s"/>放棄本回合<break time="0.2s"/>來存取該回合的接龍次數!</s><break time="0.3s"/><s>準備好了嗎?<break time="1s"/></s></p></speak>`,
		 text: '遊戲前說明',}));}
		else{
	  	conv.ask(new SimpleResponse({               
		speech: `<speak><p><s>在開始遊戲前，我需要跟您進行說明。本服務的語音辨識由Google執行。</s><s>碰到同音詞或雜音等問題時，可能會發生辨識錯誤。僅此知會你一聲!</s><break time="0.5s"/><s>此外，在你開始接龍前，你可以說<break time="0.2s"/>重新開始<break time="0.2s"/>讓我重新想一個成語作為開頭。</s><s>當你想結束挑戰時，可以說<break time="0.2s"/>放棄本回合<break time="0.2s"/>來存取該回合的接龍次數!</s><break time="0.3s"/><s>準備好了嗎?<break time="1s"/></s></p></speak>`,
		 text: '遊戲前說明',}));}
       ...
       input_list.push(sys_word);//將字詞存入佇列
...
//「開始遊戲」啟動詞判斷
  if(menu===true&&end_game===false&&question_output===false){
      ...
      if(input==='開始挑戰'){ menu=false;question_output=true;end_game=false;worngcount=0;}
  }

  //結算畫面防呆判斷
  if(menu===false&&end_game===true&&question_output===false){
     if(inputarray.indexOf(input)!==-1){
     menu=false;end_game=false;question_output=true;Total_Count=0;input_list=[];start_game=false;worngcount=0;}
  }
  
  if(menu===false&&end_game===false&&question_output===true){
	  
  if((input==='重新開始'||input==='🔄 重新開始')&&Total_Count===0){input_list=[];start_game=false;worngcount=0;}
	...
	conv.ask(new Suggestions('🔄 重新開始','放棄本回合'));
        ...
        conv.ask(new SimpleResponse({               
			speech: `<speak><p><s>輸入錯誤!</s><s>你說的<break time="0.2s"/>${input}<break time="0.2s"/>不是成語喔!</s></p></speak>`,
			text: '「'+input+'」不是成語喔 ❌',}));
		...
		if(conv.input.type==="VOICE"){  //如果輸入是語音，則顯示錯誤處理方法
			conv.ask(new Table({
					title: '『'+sys_word+'』',
					subtitle:'請輸入以「'+last_word+'」開頭的成語',
					 rows: [ {cells: ['〈錯誤說明〉\nGoogle語音辨識可能發生錯誤，或是我無法在《成語典》上找到該成語，你可以嘗試：\n• 試著再說一次\n• 透過鍵盤輸入欲表達的成語\n• 向開發者回報來改善比對結果'],dividerAfter: false,},]})); 
		}else{
			conv.ask(new BasicCard({   
				title: '『'+sys_word+'』',
				subtitle:'請輸入以「'+last_word+'」開頭的成語',
			...
			var syshelp="";
			output_array=idiom_library[last_word]; //進入字詞庫取得對應字詞
			syshelp=output_array[parseInt(Math.random()*(output_array.length-1))];
			conv.ask(new Table({
					title: '『'+sys_word+'』',
					subtitle:'請輸入以「'+last_word+'」開頭的成語',
					 rows: [ {cells: ['輔助功能 (Beta)\n為了協助你順利進行，\n你可使用下方的建議卡片輸入我所提供的成語!'],dividerAfter: false,},]})); 
			...
			worngcount=0;
		if(last_word!==first_word){repeat=false;
		
        conv.ask(new SimpleResponse({               
			speech: `<speak><p><s>輸入錯誤!</s><s>請說以<break time="0.2s"/>${last_word}<break time="0.2s"/>為開頭的成語喔!</s></p></speak>`,
			text: '你說錯成語囉 ❌',}));
		 conv.ask(new BasicCard({   
			title: '『'+sys_word+'』',
			subtitle:'請輸入以「'+last_word+'」開頭的成語',
			text:'_[!]隨時都能退出本回合結算成績_',}));
		...
		Total_Count++;				
        if(input_list.indexOf(input)===-1){   //偵錯看是否輸入到重複的詞

		input_list.push(input);//將字詞存入佇列
		input_word=input_init.pop();
		output_array=idiom_library[input_word]; //進入字詞庫取得對應字詞
