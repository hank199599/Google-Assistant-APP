    ...
    if (conv.user.verification === 'VERIFIED') {
        if (conv.screen) {
            ...
            conv.ask(new BasicCard({
                image: new Image({ url: 'https://raw.githubusercontent.com/hank199599/Google-Assistant-APP/master/%E8%A9%9E%E8%AA%9E%E6%8E%A5%E9%BE%8D/assets/yTy9fks.jpg', alt: 'Pictures', }),
                title: '遊戲規則',
        ...
        conv.close(new BasicCard({
            image: new Image({ url: 'https://raw.githubusercontent.com/hank199599/Google-Assistant-APP/master/%E8%A9%9E%E8%AA%9E%E6%8E%A5%E9%BE%8D/assets/assistant_48dp.png', alt: 'Pictures', }),
            title: '錯誤：您需要進行設定',
    ...
    if (conv.user.verification === 'VERIFIED') {
        if (menu === false && end_game === false && question_output === true && reported === false) {
            ...
            if (start_game === false) {
                ...
                input_list.push(sys_word); //將詞彙存入佇列
                conv.speechBiasing = text_library[last_word];
                ...
                //轉換字串中的異體字
                if (last_word === "溼") { input = replaceString(input, '濕', '溼'); } else if (last_word === "台") { input = replaceString(input, '臺', '台'); } else if (last_word === "臺") { input = replaceString(input, '台', '臺'); } else if (last_word === "暮") { input = replaceString(input, '目', '暮'); } else if (last_word === "剝") { input = replaceString(input, '剝離', '玻璃'); } else if (last_word === "刻") { input = replaceString(input, '可', '刻'); } else if (last_word === "刻") { input = replaceString(input, '科', '刻'); } else if (last_word === "例") { input = replaceString(input, '栗', '例'); } else if (last_word === "由") { input = replaceString(input, '魷魚', '由於'); } else if (last_word === "寢") { input = replaceString(input, '請示', '寢室'); } else if (last_word === "利") { input = replaceString(input, 'Z1', '利益'); } else if (last_word === "州") { input = replaceString(input, '周', '州'); } else if (last_word === "斗") { input = replaceString(input, '鬥', '斗'); } else if (last_word === "輩") { input = replaceString(input, '備', '輩'); } else if (last_word === "豔") { input = replaceString(input, '豔', '艷'); } else if (last_word === "源") { input = replaceString(input, '圓', '源'); } else if (last_word === "詞") { input = replaceString(input, '慈惠', '詞彙'); }
            ...
            conv.close(new BasicCard({
                image: new Image({ url: 'https://raw.githubusercontent.com/hank199599/Google-Assistant-APP/master/%E8%A9%9E%E8%AA%9E%E6%8E%A5%E9%BE%8D/assets/P5FWCbe.png', alt: 'Pictures', }),
                title: '錯誤：您需要進行設定',
...
app.intent('結束挑戰', (conv, { end_game }) => {
    ...
    if (typeof Total_Count !== "undefined") {
        ...
        if (conv.screen) {
            ...
            conv.ask(new BasicCard({
                image: new Image({ url: 'https://raw.githubusercontent.com/hank199599/Google-Assistant-APP/master/%E8%A9%9E%E8%AA%9E%E6%8E%A5%E9%BE%8D/assets/2oOhmvs.jpg', alt: 'Pictures', }),
