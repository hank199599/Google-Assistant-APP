...
app.intent('問題產生器', (conv, { input }) => {
    ...
    if (menu === true && end_game === false && question_output === false && answer_input === false && end_game === false && next_question === false) {
        if (input.indexOf('快速') !== -1) { quickmode = true; } else { quickmode = false; }
    ...
    if (menu === false && question_output === false && answer_input === false && end_game === true && next_question === false) {
        ...
        if (inputarray.indexOf(input) !== -1) {
            if (quickmode === true) { if (input.indexOf('試試一般模式') !== -1) { quickmode = false; } } else { quickmode = false; }
    ...
    if (menu === false && question_output === true && answer_input === false && end_game === false && next_question === false) {
        ...
        Total_Count++;
        Question_Title_Output = Question_Title;
        Answer_A_Output = Answer_A;
        Answer_B_Output = Answer_B;
        Answer_C_Output = Answer_C;
        Answer_D_Output = Answer_D;
        ...
        if (quickmode === true) { conv.ask(new Suggestions('⚡ 重新快速模式', '🎮 試試一般模式', '👋 掰掰')); } else { conv.ask(new Suggestions('🔄 重新開始', '👋 掰掰')); }
