...
return new Promise(function(resolve,reject){
	...
	if(input.length===1){conv.user.storage.spell=[origin_data.stroke_count,origin_data.radical,origin_data.stroke_count];}
	else{conv.user.storage.spell=[]}
	 }
	else{
	...
	for(i=0;i<temp_array.definitions.length;i++){
		...
		if(temp_array.definitions[i].def.indexOf('同「')!==-1){link_array=link_array+temp_array.definitions[i].def+'、';}
		else if(temp_array.definitions[i].def.indexOf('通「')!==-1){link_array=link_array+((temp_array.definitions[i].def).split('通「')[1]).split('」')[0]+'、';}
		else if(temp_array.definitions[i].def.indexOf('見「')!==-1){link_array=link_array+((temp_array.definitions[i].def).split('見「')[1]).split('」條')[0]+'、';}
	...
	else if(radical_output==="⼢"){radical_output="雖";}
	...
	if(suggest_list[input]!==undefined){return_array=[suggest_list[input],true];}
	else{
	...
	conv.ask(new SimpleResponse({speech:`<speak><p><s>我找不到你要查詢的詞彙</s><s>不過有幾個類似的條目供你參考。</s></p></speak>`,
								text:"找不到這個詞彙喔!\n但有幾個類似的條目。"}));}                 		
	...
	if(hasWebBrowser){
        conv.ask(new BasicCard({display: 'CROPPED',
        image: new Image({url:'https://i.imgur.com/RrqFkIp.png',alt:'Pictures',}),
        }));
	}else{
        conv.ask(new BasicCard({display: 'CROPPED',
...
app.intent('多音字查詢', (conv,{character}) => {
	...
	for(i=0;i<temp_array[num].definitions.length;i++){
		...
		if(temp_array[num].definitions[i].def.indexOf('同「')!==-1){link_array=link_array+((temp_array[num].definitions[i].def).split('同「')[1]).split('」')[0]+'、';}
		else if(temp_array[num].definitions[i].def.indexOf('通「')!==-1){link_array=link_array+((temp_array[num].definitions[i].def).split('通「')[1]).split('」')[0]+'、';}
		else if(temp_array[num].definitions[i].def.indexOf('見「')!==-1){link_array=link_array+((temp_array[num].definitions[i].def).split('見「')[1]).split('」條')[0]+'、';}
