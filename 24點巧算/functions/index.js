...
var wrong_list = ["很抱歉，我不太清楚你的意思", "我剛剛恍神了，再說一次好嗎?", "麻煩你再說一次", "不好意思，我不太明白", "不好意思，fire可以重複一下剛剛說的話嗎?"];
...
app.intent('選擇難度完成', (conv, params, option) => {
	...
	conv.speechBiasing = ['加法', '減法', '乘法','除法'];  //告知Google助理偏好的語音辨識結果
	...
	if (operator_list.length < 3) {
		...
		if (options.indexOf(operator) !== -1) {
			...
			if (operator_list.length === 3) {
				...
				conv.ask(new Suggestions(options));
				conv.ask(new Suggestions("重新作答","顯示答案"));
				conv.contexts.set(Contexts.ReAns, 1);
				conv.speechBiasing = ['加法', '減法', '乘法','除法'];  //告知Google助理偏好的語音辨識結果
...
app.intent('顯示答案', (conv) => {
	...
	var count = conv.user.storage.count;
	var speak_out={
		speech: `<speak><p><s>沒問題，以下是這個數字組合的其中一種解法</s><s>想再挑戰看看嗎?</s></p></speak>`,
		text: '沒問題，\n以下是這個數字組合可能的算式'
	}

	if(count===1){speak_out.speech=`<speak><p><s>好的!下面是這個數字組合的唯一解法</s><s>想再試一次嗎?</s></p></speak>`}

	conv.ask(new SimpleResponse(speak_out));
...
app.intent('重新輸入答案', (conv) => {
	...
	conv.ask(new Suggestions(options));
	if (operator_list.length > 0){conv.ask(new Suggestions("重新作答"));}
	conv.speechBiasing = ['加法', '減法', '乘法','除法'];  //告知Google助理偏好的語音辨識結果
