...
var teansform_library = require('./cjst.json');
...
if(conv.user.verification === 'VERIFIED'){
  if(conv.screen){
	...
	conv.ask(new BasicCard({   
			image: new Image({url:'https://i.imgur.com/vwqZn8V.jpg',alt:'Pictures',}),
			title: '遊戲規則',
   ...
   conv.ask(new Suggestions('🎮 開始遊戲','👋 掰掰'));
	   ...
	   input_word=""; last_word=""; first_word="";
		...
		last_word=teansform_library[temp[sys_word.length-1][0]];
...
//「開始遊戲」啟動詞判斷
  if(menu===true&&end_game===false&&question_output===false&&reported===false){
      ...
      if(input==='開始遊戲'){ menu=false;question_output=true;end_game=false;}
  }
  ...
  if((input==='重新開始'||input==='🔄 重新開始')&&Total_Count===0){input_list=[];start_game=false;reported=false;jumpcount=0;wrong_count=0;}
	...
	if(start_game===false){
	 ...
	 conv.ask(new BasicCard({   
		...
		buttons: new Button({title:'在《萌典》上看這個詞彙的條目',
	...
	conv.ask(new Suggestions('🔄 重新開始','跳過這個詞','放棄本回合'));
		...
		first_word=teansform_library[temp[0][0]];
	   ...
	   if(input_list.indexOf(input)===-1){   //檢查輸入的詞彙是否已重複
		...
		var temp=pinyin(input, {style: pinyin.STYLE_NORMAL,heteronym: false});
		input_word=teansform_library[temp[input.length-1][0]];
...
app.intent('結束挑戰', (conv,{end_game}) => {
 ...
 jumpcount=conv.user.storage.jumpcount;
  ...
  conv.ask(new BasicCard({   
        image: new Image({url:'https://i.imgur.com/eOAyfqS.jpg',alt:'Pictures',}),
	...
	if(start_game===false){
	 ...
	 conv.ask(new Suggestions('🔄 重新開始','放棄本回合'));
		...
		wrong_array=[`<speak><p><s>錯誤!</s><s>不能將中文以外的字符作為輸入!</s></p></speak>`,
			`<speak><p><s>包含非法字元，母湯喔!</s></p></speak>`,
			`<speak><p><s>安ㄋㄟˉ母湯，請換一個詞來試看看</s></p></speak>`,
			`<speak><p><s>你的輸入混入怪怪的東西，請換一個!</s></p></speak>`,						
			`<speak><p><s>這個詞彙裡有怪怪的東西，請換一個。</s></p></speak>`,];
		...
		wrong_array=[`<speak><p><s>想的好，但是自首不是${last_word}喔!再想一個八!</s></p></speak>`,
			`<speak><p><s>字首好像不太對喔，試著換一個八!</s></p></speak>`,
			`<speak><p><s>字首對ㄅㄨˊ上呢，再想一想後頭可以接什麼詞彙!</s></p></speak>`,
			`<speak><p><s>字首不太對，換一個試看看!</s></p></speak>`,						
			`<speak><p><s>這個詞彙的字首不是我要的，請換一個。</s></p></speak>`,];
	 ...
	 if(typeof output_array==="undefined"){
		...
		var temp=pinyin(input, {style: pinyin.STYLE_NORMAL,heteronym: false});
		last_word=teansform_library[temp[0][0]];
        output_array=text_library[last_word]; //進入詞彙庫取得對應詞彙
		sys_suggest=output_array[parseInt(Math.random()*(output_array.length-1))];
