...
i18n.configure({
	locales: ['zh-TW', 'zh-HK', 'ja-JP', 'ko-KR', 'th'],
	directory: __dirname + '/locales',
	defaultLocale: 'zh-TW',
});
app.middleware((conv) => {
	var temp = conv.user.locale;
	if (temp.indexOf('th') !== -1) { temp = "th" }
	i18n.setLocale(temp);
});
	...
	if (permissionGranted) {
		...
		if (requestedPermission === 'DEVICE_PRECISE_LOCATION') {
			...
			return new Promise(function (resolve, reject) {
				...
				Source = origin_data.attributions[0].name.split('(')[0];
				...
				if (AQI >= 0 && AQI <= 50) {
					conv.ask(new SimpleResponse({
						speech: `<speak><p><s>${i18n.__('output1', AQI)}</s><s>${i18n.__('output2')}</s></p></speak>`,
					...
					conv.ask(new SimpleResponse({
						speech: `<speak><p><s>${i18n.__('output1', AQI)}</s><s>${info}</s></p></speak>`,
						text: i18n.__('info_test')
					}));
				}
				...
				conv.close(new BasicCard({
					...
					title: origin_data.city.name, display: 'CROPPED',
					subtitle: Status + output_title,
					text: info_output + '  \n  \nPM₁₀ ' + PM10 + '(AQI) • PM₂.₅ ' + PM25 + '(AQI) • ' + i18n.__('O3') + ' ' + O3 + '(AQI)  \n' + i18n.__('from', Source),
					buttons: new Button({title: i18n.__('Button_Title'),url:origin_data.city.url}),
				}));
