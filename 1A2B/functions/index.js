...
function ranGuess() {
  ...
  else if (output.length === 2) { output = "00" + output; }
  else if (output.length === 1) { output = "000" + output; }
...
function analysis(you_guess, sys_think) {
  ...
  //千位數判斷
  if (sys_split[0] === your_split[0]) { A_count++; array[0] = 'A'; }
  else if (sys_split[0] === your_split[1]) { B_count++; array[1] = 'B'; }
  else if (sys_split[0] === your_split[2]) { B_count++; array[2] = 'B'; }
  else if (sys_split[0] === your_split[3]) { B_count++; array[3] = 'B'; }

  //百位數判斷

  if (sys_split[1] === your_split[0]) { B_count++; array[0] = 'B'; }
  else if (sys_split[1] === your_split[1]) { A_count++; array[1] = 'A'; }
  else if (sys_split[1] === your_split[2]) { B_count++; array[2] = 'B'; }
  else if (sys_split[1] === your_split[3]) { B_count++; array[3] = 'B'; }

  //十位數判斷

  if (sys_split[2] === your_split[0]) { B_count++; array[0] = 'B'; }
  else if (sys_split[2] === your_split[1]) { B_count++; array[1] = 'B'; }
  else if (sys_split[2] === your_split[2]) { A_count++; array[2] = 'A'; }
  else if (sys_split[2] === your_split[3]) { B_count++; array[3] = 'B'; }

  //個位數判斷
  if (sys_split[3] === your_split[0]) { B_count++; array[0] = 'B'; }
  else if (sys_split[3] === your_split[1]) { B_count++; array[1] = 'B'; }
  else if (sys_split[3] === your_split[2]) { B_count++; array[2] = 'B'; }
  else if (sys_split[3] === your_split[3]) { A_count++; array[3] = 'A'; }
...
function explain(A, B, else_count) {
  if (A === 0 && B === 0) { return i18n.__('explaine_1', (10 - else_count)); }
  else if (A >= 1 && A <= 2 && B === 0) { return i18n.__('explaine_2', A, else_count); }
  else if (A === 2 && B === 2) { return i18n.__('explaine_3'); }
  else if (A === 0 && B <= 3) { return i18n.__('explaine_4', B); }
  else if (A === 3 && B === 0) { return i18n.__('explaine_5', A); }
  else if (A === 0 && B === 4) { return i18n.__('explaine_6', A); }
  else if (A === 1 && B === 3) { return i18n.__('explaine_3'); }
  else { return i18n.__('explaine_7', A, B, else_count); }
}
  ...
  if (conv.screen) {
    ...
    if (conv.user.last.seen) {
      ...
      conv.ask(new BasicCard({
        ...
        title: i18n.__('Welcome_Title'), display: 'CROPPED',
...
app.intent('輸入數字', (conv, { any }) => {
  ...
  if (conv.user.storage.guess_count !== undefined) {
    ...
    if (isNaN(number) === false) {
      ...
      if (you_guess.length === 3) { you_guess = "0" + you_guess; }
      else if (you_guess.length === 2) { you_guess = "00" + you_guess; }
      else if (you_guess.length === 1) { you_guess = "000" + you_guess; }
      ...
      if (A_count <= 3) {
        ...
        conv.ask(new BasicCard({
          image: new Image({ url: 'https://imgur.com/7qDUUEq.jpg', alt: 'Pictures', }), display: 'CROPPED',
...
app.intent('解釋意思', (conv) => {
  ...
  if (conv.user.storage.guess_count === 0) {
    ...
    if (input === i18n.__('error_record')) {
      ...
      conv.ask(new SimpleResponse({ speech: `<speak><p><s>${i18n.__('hint1','<break time="0.15s"/><say-as interpret-as="characters">'+input+'</say-as><break time="0.15s"/>')}${explained}</s></p></speak>`, text: i18n.__('hinttext1') }));
      conv.ask(new BasicCard({
      ...
      conv.ask(new SimpleResponse({ speech: `<speak><p><s>${i18n.__('hint1','<break time="0.15s"/><say-as interpret-as="characters">'+input+'</say-as><break time="0.15s"/>')}${explained}</s></p></speak>`, text: i18n.__('hinttext2') }));
      conv.ask(new Table({
        title: input + '   (' + A_count + 'A' + B_count + 'B)',
        subtitle: i18n.__('Hint_subtitle', A_count, A_count, B_count, B_count, else_count),
...
app.intent('教學模式', (conv, { any }) => {
  ...
  if (try_count === 0) { any = String(sys_error1) + String(sys_split[0]) + String(sys_split[2]) + String(sys_error2); }
  else if (try_count === 1) { any = String(sys_error1) + String(sys_split[1]) + String(sys_split[2]) + String(sys_error2); }
  else if (try_count === 2) { any = String(sys_error1) + String(sys_split[0]) + String(sys_split[1]) + String(sys_error2); }
  else if (try_count === 3) { any = String(sys_split[0]) + String(sys_split[1]) + String(sys_split[2]) + String(sys_error2); }
