...
var pronunciation_list = require('./pronunciation.json');
var subject_list = require('./subject.json');
...
app.intent('查詢字典', (conv, { input }) => {
    ...
    if (replace_list[input] !== undefined) { input = replace_list[input]; }
    ...
    return new Promise(function(resolve, reject) {
        ...
        for (i = 0; i < origin_data.heteronyms.length; i++) {
            var temp = origin_data.heteronyms[i].bopomofo;
            if (temp !== undefined) { temp_array.push(origin_data.heteronyms[i]); }
        }
        ...
        if (temp_array.length > 1) {
            ...
            for (i = 0; i < temp_array.length; i++) {
                var bopomofo_temp = temp_array[i].bopomofo.replace(/[\（|\語|\音|\）|\讀|\又|]/g, "");
                conv.ask(new Suggestions(bopomofo_temp));
                bopomofo_array.push(bopomofo_temp);
            }
            conv.user.storage.bopomofo_array = bopomofo_array;
            conv.user.storage.heteronyms = temp_array;
            conv.user.storage.input_charactor = input;
            if (input.length === 1) { conv.user.storage.spell = [origin_data.stroke_count, origin_data.radical, origin_data.stroke_count]; } else { conv.user.storage.spell = [] }
        } else {
            temp_array = temp_array[0];
            bopomofo = temp_array.bopomofo;
            ...
            for (i = 0; i < temp_array.definitions.length; i++) {

                if (temp_array.definitions[i].def.indexOf('同「') !== -1) { link_array = link_array + temp_array.definitions[i].def + '、'; } else if (temp_array.definitions[i].def.indexOf('通「') !== -1) { link_array = link_array + ((temp_array.definitions[i].def).split('通「')[1]).split('」')[0] + '、'; } else if (temp_array.definitions[i].def.indexOf('見「') !== -1) { link_array = link_array + ((temp_array.definitions[i].def).split('見「')[1]).split('」條')[0] + '、'; }

                var temp = temp_array.definitions[i].def;
                ...
                if (isChn(temp) === true) {
                    ...
                    if (temp_array.definitions.length > 1) { definitions = definitions + (j) + '.**'; } else { definitions = definitions + '**'; }
                    var process = temp_array.definitions[i].def;
                    ...
                    if (temp_array.definitions[i].example !== undefined) {
                        var process = temp_array.definitions[i].example[0];
                        process = replaceString(process, '如：', '㊟');
                        definitions = definitions + '*' + process + '*  \n';

                        example_array = example_array + temp_array.definitions[i].example + '、';
                        example_array = example_array.replace(/[\「|\。|\」|]/g, "");
                    } //蒐集範例
                    j++;
                }
                if (temp_array.definitions[i].synonyms !== undefined) { synonyms = synonyms + temp_array.definitions[i].synonyms + ','; } //蒐集同義詞
                if (temp_array.definitions[i].antonyms !== undefined) { antonyms = antonyms + temp_array.definitions[i].antonyms + ','; } //蒐集反義詞
                if (temp_array.definitions[i].link !== undefined) {
                    var k = 0;
                    for (k = 0; k < temp_array.definitions[i].link.length; k++) {
                        link_array = link_array + temp_array.definitions[i].link[k] + '、';
                        link_array = link_array.replace(/[\同|\通|\見|\條|\「|\。|\」|]/g, "");
                    }
                } //蒐集深入連結條例
            }
            ...
            if (radical !== undefined) {
                ...
                if (pronunciation_list[radical] !== undefined) { radical_output = pronunciation_list[radical] } else { radical_output = radical; }
                ...
                if (radical !== input) { conv.ask(new Suggestions(radical)); } else { conv.ask(new Suggestions('✎講解「二一四部首」')); }
                ...
                if (input.length < 4) {
                    ...
                    for (m = 0; m < input_array.length; m++) {
                        if (compare_array[input_array[m] + input_array[m + 1] + input_array[m + 2]] === "true") {
                    ...
                    for (i = 0; i < return_array.length; i++) {
                        conv.ask(new Suggestions(return_array[i]));
                    }

                }

                for (var i = 0; i < Object.keys(subject_list).length; i++) {
                    if (input.indexOf(Object.keys(subject_list)[i]) !== -1) {
                        var temp = subject_list[Object.keys(subject_list)[i]] //隨機選取近似詞彙的陣列，並自其中隨機挑選五個
                        for (var j = 0; j < 5; j++) {
                            var rand = Math.random();
                            rand *= temp.length;
                            rand = Math.floor(rand);

                            if (temp[rand] !== input) { conv.ask(new Suggestions(temp[rand])); }
            ...
            if (synonyms.length !== 0) {
                ...
                for (i = 0; i < synonyms.length; i++) {
                    if (synonyms[i].length > 0) { conv.ask(new Suggestions(synonyms[i])); }
                }
            }
            if (antonyms.length !== 0) {
                antonyms = antonyms.split(',');
                for (i = 0; i < antonyms.length; i++) {
                    if (antonyms[i].length > 0) { conv.ask(new Suggestions(antonyms[i])); }
                }
            }
        ...
        if (error === "Parse error: SyntaxError: Unexpected token < in JSON at position 0" && input.length > 0) {
            conv.ask(new SimpleResponse({ speech: `<speak><p><s>很抱歉，目前萌典的伺服器回應發生問題</s><s>因此無法進行查詢，敬請見諒!</s></p></speak>`, text: "獲取資訊發生錯誤" }));

            conv.close(new BasicCard({
                image: new Image({ url: "https://dummyimage.com/1037x539/ef2121/ffffff.png&text=錯誤", alt: 'Pictures', }),
                title: "無法完成API請求",
                text: "請靜待《萌典》伺服器修復，造成不便敬請見諒!",
                buttons: new Button({ title: '在《萌典》上查看這個詞彙', url: 'https://www.moedict.tw/' + encodeURIComponent(input), display: 'CROPPED', }),
            }));
        } else {
            if (input.length > 0) {
                ...
                return new Promise(function(resolve, reject) {
                    ...
                    if (suggest_list[input] !== undefined) { return_array = [suggest_list[input], true]; } else {
                        input_array = input.split('');
                        for (m = 0; m < input_array.length; m++) {
                    ...
                    var suggest_data = origin_data[0];

                    if (origin_data[1] === false) {
                        conv.ask(new SimpleResponse({
                            speech: `<speak><p><s>我找不到你要查詢的詞彙</s><s>你可以試著輸入分拆的詞來查詢各自的意思。</s></p></speak>`,
                            text: "找不到這個詞彙喔!\n可以嘗試輸入單詞來查詢。"
                        }));
                    } else {
                        conv.ask(new SimpleResponse({
                            speech: `<speak><p><s>我找不到你要查詢的詞彙</s><s>不過有幾個類似的條目供你參考。</s></p></speak>`,
                            text: "找不到這個詞彙喔!\n但有幾個類似的條目。"
                        }));
                    }
                    ...
                    for (i = 0; i < suggest_data.length; i++) {
                        conv.ask(new Suggestions(suggest_data[i]));
                    }
                    conv.ask(new Suggestions('➥結束查詢'));
                }).catch(function(error) {
...
app.intent('多音字查詢', (conv, { character }) => {
    ...
    temp_array = conv.user.storage.heteronyms;
    ...
    var spell = conv.user.storage.spell;
    ...
    if (num !== -1) {
        ...
        for (i = 0; i < temp_array[num].definitions.length; i++) {

            if (temp_array[num].definitions[i].def.indexOf('同「') !== -1) { link_array = link_array + ((temp_array[num].definitions[i].def).split('同「')[1]).split('」')[0] + '、'; } else if (temp_array[num].definitions[i].def.indexOf('通「') !== -1) { link_array = link_array + ((temp_array[num].definitions[i].def).split('通「')[1]).split('」')[0] + '、'; } else if (temp_array[num].definitions[i].def.indexOf('見「') !== -1) { link_array = link_array + ((temp_array[num].definitions[i].def).split('見「')[1]).split('」條')[0] + '、'; }

            var temp = temp_array[num].definitions[i].def;
            ...
            if (isChn(temp) === true) {
                ...
                if (temp_array[num].definitions.length > 1) { definitions = definitions + (j) + '.**'; } else { definitions = definitions + '**'; }
                var process = temp_array[num].definitions[i].def;
                //if(isBrackets(process)!==-1){process=process.split('（')[0];}
                process = replaceString(process, '如：', '如')
                process = replaceString(process, '。如', '。**  \n㊟*')
                process = replaceString(process, '<1>', '');
                ...
                if (temp_array[num].definitions[i].example !== undefined) {
                    var process = temp_array[num].definitions[i].example[0];
                    process = replaceString(process, '如：', '㊟');
                    definitions = definitions + '*' + process + '*  \n';

                    example_array = example_array + temp_array[num].definitions[i].example + '、';
                    example_array = example_array.replace(/[\「|\。|\」|]/g, "");
                } //蒐集範例
                j++;
            }
            if (temp_array[num].definitions[i].synonyms !== undefined) { synonyms = synonyms + temp_array[num].definitions[i].synonyms + ','; } //蒐集同義詞
            if (temp_array[num].definitions[i].antonyms !== undefined) { antonyms = antonyms + temp_array[num].definitions[i].antonyms + ','; } //蒐集反義詞
            if (temp_array[num].definitions[i].link !== undefined) {
                var k = 0;
                for (k = 0; k < temp_array[num].definitions[i].link.length; k++) {
                    link_array = link_array + temp_array[num].definitions[i].link[k] + '、';
                    link_array = link_array.replace(/[\同|\通|\見|\條|\「|\。|\」|]/g, "");
                }
            } //蒐集深入連結條例
        }

        stroke_count = spell[0];
        radical = spell[1];
        stroke_count = spell[2];
