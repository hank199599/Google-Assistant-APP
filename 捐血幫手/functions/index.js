...
function blood_store() {
	...
	if (hour_now === 23 && minute_now >= 45 && minute_now <= 59) {
	//if(minute_now <= 59){
...
function status_generator(state) {
	...
	else if (state === "empty") { return "不足"; }
	else { return "──"; }
}

function status_picker(locate, blood_type) {
	var thearray = [];
	if (locate === "台北") { thearray = Taipei_array }
	else if (locate === "新竹") { thearray = Hsinchu_array }
	else if (locate === "台中") { thearray = Taichung_array }
	else if (locate === "台南") { thearray = Tainan_array }
	else if (locate === "高雄") { thearray = Kaohsiung_array }

	return thearray[type_array.indexOf(blood_type)]
}
...
function status_outputer(input) {
	...
	for (i = 0; i < 4; i++) {
		if (input[i] === "full") { array1.push('<say-as interpret-as="characters">' + type_array[i] + '</say-as>型'); }
		else if (input[i] === "medium") { array2.push('<say-as interpret-as="characters">' + type_array[i] + '</say-as>型'); }
		else if (input[i] === "empty") { array3.push('<say-as interpret-as="characters">' + type_array[i] + '</say-as>型'); }
		else { array4.push('<say-as interpret-as="characters">' + type_array[i] + '</say-as>型'); }
	}
...
app.intent('預設歡迎語句', (conv) => {
	return new Promise(
		...
		function (resolve) {
			...
			Taipei_array = final_data.Taipei;
			Hsinchu_array = final_data.Hsinchu;
			Taichung_array = final_data.Taichung;
			Tainan_array = final_data.Tainan;
			Kaohsiung_array = final_data.Kaohsiung;
			PublishTime = final_data.PublishTime;
	...
	if (index_array.indexOf(option) !== -1) {
		return new Promise(
			...
			function (resolve) {
				...
				conv.ask(new Table({
					title: option + '捐血中心',
					subtitle: '更新時間 • ' + PublishTime,
					columns: [{ header: '血型', align: 'CENTER', }, { header: '庫存量', align: 'CENTER', }, { header: '狀態', align: 'CENTER', }],
					rows: [
						{ cells: ['A', status_generator(temp[0]), stock_generator(temp[0])], },
						{ cells: ['B', status_generator(temp[1]), stock_generator(temp[1])], },
						{ cells: ['O', status_generator(temp[2]), stock_generator(temp[2])], },
						{ cells: ['AB', status_generator(temp[3]), stock_generator(temp[3])], },],
					buttons: new Button({
	...
	return new Promise(
		...
		function (resolve) {
			...
			if (type_array.indexOf(blood_type) !== -1 && index_array.indexOf(locate) !== -1) {
				...
				conv.ask(new Table({
					title: locate + '捐血中心',
					subtitle: '更新時間 • ' + PublishTime, columns: [{ header: '血型', align: 'CENTER', }, { header: '庫存量', align: 'CENTER', }, { header: '狀態', align: 'CENTER', }],
					rows: [
						{ cells: ['A', status_generator(temp[0]), stock_generator(temp[0])], },
						{ cells: ['B', status_generator(temp[1]), stock_generator(temp[1])], },
						{ cells: ['O', status_generator(temp[2]), stock_generator(temp[2])], },
