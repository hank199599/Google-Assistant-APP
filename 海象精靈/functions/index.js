...
var output_title="";
var PublishTime="";
var origin_data=[];var SiteName_list=[];
var data_today=[];var data_tomorrow=[];var data_after=[];
...
function weather_report_set(){
   ...
   //取得概況報告
	...
	station_list=[];data_today=[];data_tomorrow=[];data_after=[];
	var time = new Date();
...
function(resolve){
	...
	word1=sea_area[parseInt(Math.random()*149)];
        ...
        image: new Image({url:"https://i.imgur.com/vrFdxK6.jpg",alt:'Pictures',}),
        title:"歡迎使用",
  ...
  conv.ask(new Carousel({
	  ...
	  items: {
		'台灣近海': {
	      ...
	      image: new Image({url: "https://i.imgur.com/05cUMdT.jpg",alt: 'Image alternate text',}),},
		'遠洋地區': {
		  title: '遠洋漁業',
		  description: '台灣附近公海',
	      image: new Image({url: "https://i.imgur.com/nIv7Dq8.jpg",alt: 'Image alternate text',}),},
	},}));
...
function(resolve){
	...
	if(day_select==="今天"){	 temp_list=data_list[station_array.indexOf(station)];			title=station;subtitle=getDay(0);}
	else if(day_select==="明天"){temp_list=data_list_tomorrow[station_array.indexOf(station)];  title=station;subtitle=getDay(1);}
	else if(day_select==="後天"){temp_list=data_list_nest_day[station_array.indexOf(station)];  title=station;subtitle=getDay(2);}
	...
	for(i=0;i<day_array.length;i++){
		var days=[];
		if(day_array[i]!==day_select){conv.ask(new Suggestions(day_array[i]+'呢?'));days.push(day_array[i]);}
...
app.intent('快速呼叫選單', (conv,{select_area}) => {
    ...
    if(conv.screen){conv.ask(new SimpleResponse({               
		 ...
		 var output="";
		 	if(select_area==="台灣近海"){output="台灣北部、台灣西半部、台灣東半部、離島區域";}
			else if(select_area==="遠洋地區"){output="台灣周圍、北部海域、南海地區";}
