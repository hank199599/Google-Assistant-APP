...
var explain_dict = require('./toeic.json');
var questions = Object.keys(explain_dict);
var accent = '';
var accent_list = ["EN-US", "EN-UK", "EN-IN", "EN-AU"]; //可選取的口音清單
var accent_display = { "EN-US": "🇺🇸 美國腔", "EN-UK": "🇬🇧 英國腔", "EN-IN": "🇮🇳 印度腔", "EN-AU": "🇦🇺 澳洲腔", "隨機": "隨機出題" }; //可選取的口音清單
...
app.intent('開始遊戲', (conv, params, option) => {
    ...
    //尋找單字對應的解釋
    var array = spliter.split(explain_dict[question])
    var temp = [];
...
app.intent('進行猜測', (conv, { input }) => {
    ...
    if (flag === false && guess_count <= 2) {
        ...
        conv.ask(new SimpleResponse({
            speech: `<speak><p><s><audio src="${audio_url}"/><break time="0.5s"/>的意思是${explain_dict[conv.user.storage.question]}</s></p></speak>`,
            text: "以下是這個單詞的意思"
        }));
        conv.ask(new BasicCard({
            title: conv.user.storage.question,
            subtitle: explain_dict[conv.user.storage.question],
...
app.intent('重複題目', (conv) => {
    ...
    conv.ask(new SimpleResponse({
        speech: `<speak><p><s>${reply_array[parseInt(Math.random() * (reply_array.length - 1))]}，仔細聽好囉<break time="0.5s"/><audio src="${audio_url}"/></s></p></speak>`,
        text: "沒問題，仔細聽好囉!"
    }));
...
app.intent('結算成績', (conv) => {
    ...
    for (var i = 0; i < Q_list.length; i++) {
        rowsarray.push({
            cells: [String(i + 1), Q_list[i], explain_dict[Q_list[i]]],
            dividerAfter: false,
        });
    }
    ...
    conv.ask(new SimpleResponse({
        speech: `<speak><p><s>在這回合中，我們一共學習到五個單字</s><break time="0.5s"/><s>分別是${output_word}</s></p></speak>`,
        text: "下面是我們在這回合中所學習到的單字"
    }));

    conv.ask(new Table({
        title: "這回合的題目列表",
        columns: [{ header: '題數', align: 'CENTER', }, { header: '題目', align: 'CENTER', }, { header: '解釋', align: 'CENTER', }],
        rows: rowsarray,
    }));
